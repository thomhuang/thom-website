...
import {Category, GetCategoriesAsync, GetPostsAsync, Post} from '../../api/Posts/PostsRouter'
...
export default function Posts() {
    ...
    useEffect(() => {
        ...
        const hydrateCategoriesWithPosts = async () : Promise<Category[] | undefined> =>  {
            try {
                const categoriesResponse = await GetCategoriesAsync()

                if (categoriesResponse !== undefined) {
                    return await Promise.all(categoriesResponse?.map(async (category: Category) => {
    ...
    function DisplayPosts() {
        ...
        return categories.map(item => (<CollapsibleSection
                title={item.Category}
                content={item.Posts}
                key={item.Category}
            />))
